...
function sw_less_construct(){
	...
	$color =  revo_options()->getCpanelValue('scheme_color');
	$bd_color =  revo_options()->getCpanelValue('scheme_body');
	$bdr_color =  revo_options()->getCpanelValue('scheme_border');
	...
	if ( revo_options()->getCpanelValue('developer_mode') ){
		...
		$ya_direction = revo_options()->getCpanelValue( 'direction' );
		...
		if ( file_exists($scheme_vars) ){
			...
			try {
				...
				// $less_variables['sidebar-width'] = revo_options()->sidebar_collapse_width.'px';
