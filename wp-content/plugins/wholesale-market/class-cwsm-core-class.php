...
class CED_CWSM_Core_Class
{
	...
	*/
	public function ced_cwsm_include_required_files() {
		...
		require_once 'core/adminSettingsCore/class-cwsm-admin-settings.php';
		require_once 'core/adminSide/class-cwsm-basic-settings.php';
		require_once 'core/adminSide/class-cwsm-productListingPage-customization.php';
		require_once 'core/adminSide/class-cwsm-add-product-meta-fields.php';
		require_once 'core/frontEnd/class-cwsm-show-wholesale-price-on-frontEnd.php';
		require_once 'core/frontEnd/class-cwsm-manage-wholesale-price.php';
		require_once 'addons/min-product-qty-module/class-cwsm-min-product-qty-module.php';
		require_once 'addons/admin-suggestions/class-cwsm-admin-suggestions.php';

		/**
		* @since  1.0.8
		*/
		require_once 'core/frontEnd/class-cwsm-custom-cart-page-message.php';
		require_once 'addons/csv-import-export/class-cwsm-csv-import-export.php';
		require_once 'addons/wholesale-request/wholesale-request-addon.php';
		require_once 'addons/wholesale-advertisement/wholesale-advertisement-addon.php';
			
		/**
		* @since  1.0.12
		*/
		require_once 'addons/min-checkout-price-module/class-cwsm-min-checkout-price-module.php';

		/**
		* @since  2.0.0
		*/
		require_once 'core/frontEnd/class-cwsm-manage-add-to-cart-button.php';
		
		/**
		* @since  2.0.3
		*/
		require_once 'addons/sticky-admin-suggestions/class-cwsm-sticky-send-suggestions.php';		
	}
	 ...
	 * @link: http://www.cedcommerce.com/
	 */
	function ced_cwsm_add_plugin_row_meta($plugin_meta, $plugin_file, $plugin_data, $status)
	{
		 ...
		 */
		function enqueue_common_styles_and_scripts() {
			$screen       = get_current_screen();
  			$screen_id    = $screen ? $screen->id : '';
  			
  			if(strpos($_SERVER['REQUEST_URI'], "page=wholesale_market")) {
				wp_enqueue_style('ced_cwsm_setting_panel_css',plugins_url('assets/css/setting-panel.css',__FILE__));
			}
			else if ( in_array( $screen_id, array( 'product', 'edit-product' ) ) ) {
				wp_enqueue_style('ced_cwsm_product_panel_css',plugins_url('assets/css/product-panel.css',__FILE__), array('woocommerce_admin_styles','woocommerce_admin_menu_styles') );
			}
		}
		 ...
		 * @link http://cedcommerce.com/
		 */
		function enqueue_woocommerce_setting_section_styles_and_scripts() {			
			if( isset($_GET['page']) && $_GET['page']=='wholesale_market' && !isset($_GET['tab']) && !isset($_GET['section']) ) {
				...
				wp_enqueue_style('ced_cwsm_error_msg_backend',plugins_url('assets/css/error_msg_backend.css',__FILE__));
			}
			else if( isset($_GET['page']) && $_GET['page']=='wholesale_market' && isset($_GET['tab']) && $_GET['tab']=='ced_cwsm_basic' && !isset($_GET['section']) ) {
				wp_enqueue_script( 'custom_price_txt_script', plugins_url('assets/js/custom_price_txt_script.js',__FILE__), array('jquery'), '1.0', true );
				wp_enqueue_style('ced_cwsm_error_msg_backend',plugins_url('assets/css/error_msg_backend.css',__FILE__));
			}
			else if( isset($_GET['page']) && $_GET['page']=='wholesale_market' && isset($_GET['tab']) && $_GET['tab']=='ced_cwsm_basic' && isset($_GET['section']) && $_GET['section']=='ced_cwsm_general_module') {
				wp_enqueue_script( 'custom_price_txt_script', plugins_url('assets/js/custom_price_txt_script.js',__FILE__), array('jquery'), '1.0', true );
				wp_enqueue_style('ced_cwsm_error_msg_backend',plugins_url('assets/css/error_msg_backend.css',__FILE__));
			}
		 ...
		 * @link: http://www.cedcommerce.com/
		 */
		function ced_cwsm_load_textdomain() {
