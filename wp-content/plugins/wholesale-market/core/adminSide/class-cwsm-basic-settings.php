...
class CED_CWSM_Basic_Settings {
	...
	public function add_required_hooks_and_filters() {
		...
		if( $ced_cwsm_current_tab == 'ced_cwsm_basic' ) {
	...
	function ced_cwsm_add_sections( $sections ) {
	 ...
	 * @link  http://www.cedcommerce.com/
	 */
	function ced_cwsm_add_settings( $settings ) {	
		global $ced_cwsm_current_section;
		if( $ced_cwsm_current_section == $this->_sectionID ) {
			
			$settings = apply_filters( 'ced_cwsm_alter_'.$this->_sectionID.'_settings', array(
				'top-label' => array(
