...
class CED_CWSM_ProductListingPage_Customization {
	 ...
	 * @link  http://www.cedcommerce.com/
	 */
	function add_wholesale_price_column($columns){
	 ...
	 * @link  http://www.cedcommerce.com/
	 */	
	function show_wholesale_price_column_value( $column, $postid ) {
	    if ( $column == 'ced_cwsm_ws_price' ) {
	    	global $globalCWSM;
	    	...
	    	if($product->is_type('variable')) {
				...
				if(!empty($wholesalePriceRange)) {
					echo '<p style="color:purple;font-weight:bold;">'.$wholesalePriceRange.'</p>';
				}else{
		    	...
		    	if( $wholesalePrice ) {
			    	...
			    	echo '<p style="color:purple;font-weight:bold;">'.$wholesalePrice.'</p>';
		    	}else{
					echo '<p style="color:purple;font-weight:bold;">-</p>';
				}
	    	}	
