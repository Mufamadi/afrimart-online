...
class CED_CWSM_Custom_Cart_Page_Message {
	...
	function cwsm_trigger_update_cart_script(){
		global $globalCWSM;
		...
		if(is_cart()){
			...
			wp_enqueue_style( 'ced_cwsm_cart_page_message_style', plugins_url('css/cart_page_message_style.css',__FILE__) );
		}
	}
	 ...
	 */
	function ced_cwsm_woocommerce_before_cart()
	{
		...
		if( is_array( $cart ) && ( count( $cart ) > 0 ) ) {
		    ...
		    if(WC()->version<'3.0.0')
		    	...
		    	if( $proceed ) {
		    		$productKeyToAppend = ($variation_id != '0') ? $variation_id : $product_id;
	 ...
	 * @link  http://www.cedcommerce.com/
	 */
	function cwsm_generate_msg_about_wholesale_price_application( $product_id, $variation_id, $quantity, $price, $product_name, $cart_item, $arrayToKeepTrackOfProductsMsgAddedFor ) {
		...
		if( !isset($wholesale_price) || empty($wholesale_price) || $wholesale_price == '' || $wholesale_price == '0' )
		{
			return "";
		}
		...
		$productIDToConsider = ($variation_id != '0') ? $variation_id : $product_id;
	 ...
	 * @link  http://www.cedcommerce.com/
	 */
	function ced_cwsm_add_to_cart_fragments( $fragments ) {
		global $globalCWSM;
		//check for plugin activation and wholesale-user
		if( !$globalCWSM->is_CWSM_plugin_active() || !$globalCWSM->isCurrentUserIsWholesaleUser() ){
			return $fragments;
