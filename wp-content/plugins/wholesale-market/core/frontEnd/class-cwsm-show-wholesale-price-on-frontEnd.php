...
class CED_CWSM_Show_Wholesale_Price_On_FrontEnd {
	 ...
	 */
	function ced_cwsm_show_wholesale_price_shop_page() {
		...
		if($product->is_type('simple') || $product->is_type('grouped')) {	 //for simple product	
			...
			if(!empty($wholesalePrice))
			{
				...
				if($isMinQtyEnable == "yes")
				{
					if($minQtyPicker == CED_CWSM_PREFIX."radio_individual_minQty")
					{
				...
				if(empty($minQtyCheck) || $minQtyCheck == 0 || $isMinQtyEnable != "yes")
				{
					if(WC()->version<'3.0.0')
				{
		...
		else if($product->is_type('variable'))		//for variable product
		{
	 ...
	 * @link  http://www.cedcommerce.com/
	 */
	function ced_cwsm_show_wholesale_price_single_page()
	{
		global $globalCWSM;
		if(!$globalCWSM->isActiveUserCanSeeWholesalePrice())
			return;
		...
		if ($_product->is_type ( 'variable' )) 
		{
			...
			if(!empty($wholesalePriceRange))
			{
				...
				{	
					...
					echo '<p class="price">'.$customShopPageTxt.'</p>';
				}
			}
		} 
	 ...
	 * @link  http://www.cedcommerce.com/
	 */
	function ced_cwsm_show_variation_wholesale_price($htmlArray,$thisRef,$variationCopy)
	{
		global $globalCWSM;
		if(!$globalCWSM->isActiveUserCanSeeWholesalePrice())
			return $htmlArray;
		...
		if(empty($minQtyCheck) || $minQtyCheck == 0)
		{
			if(WC()->version<'3.0.0')
						{
