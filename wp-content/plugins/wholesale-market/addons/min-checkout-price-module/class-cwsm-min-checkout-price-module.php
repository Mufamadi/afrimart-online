...
class CED_CWSM_Min_Checkout_Price_Module {
	...
	public function ced_cwsm_min_pro_qty_module_add_hooks_and_filters() {
		...
		if(isset($ced_cwsm_enable_minCheckoutPrice) && !empty($ced_cwsm_enable_minCheckoutPrice) && $ced_cwsm_enable_minCheckoutPrice == "yes") {
	 ...
	 * @link  http://www.cedcommerce.com/
	 */
	function enqueue_style_and_script_on_adminSide() {
		$urlToUse = CED_CWSM_PLUGIN_DIR_URL.'assets/css/error_msg_bckend.css';
		if(isset($_GET['section']) && $_GET['section'] == 'ced_cwsm_min_checkout_price_module') {
			wp_enqueue_style('ced_cwsm_error_msg_backend', $urlToUse);
		}
	}
	 ...
	 * @link  http://www.cedcommerce.com/
	 */
	function enqueue_style_and_script_on_frontEnd() {
	 ...
	 * @link  http://www.cedcommerce.com/
	 */
	function woocommerce_before_cart_hidden_element() {
		global $globalCWSM;
		...
		$errorMsg = $errorMsg = $this->generateErrorMessage();
		echo '<div id="cwsm_min_checkout_price_error" style="display:none;">';
		echo '<input type="hidden" name="isQualifyMinCheckoutPriceCondition" id="isQualifyMinCheckoutPriceCondition" value="'.$isQualifyMinCheckoutPriceCondition.'">';
		echo '<div class="cwsm_custom_msg_error">'.$errorMsg.'</div>';
		echo '</div>';
	}
	 ...
	 * @link  http://www.cedcommerce.com/
	 */
	function woocommerce_add_to_cart_fragments_hidden_element( $fragments ) {
		global $globalCWSM;
		if(!$globalCWSM->isCurrentUserIsWholesaleUser()) {
			return $fragments;
		}
	 ...
	 * @link  http://www.cedcommerce.com/
	 */
	function avoid_rendering_checkout_page($returnErrorMsg=false) {
		global $globalCWSM;
		if(!$globalCWSM->isCurrentUserIsWholesaleUser()) {
			return ;
		}
	 ...
	 * @link  http://www.cedcommerce.com/
	 */
	function isQualifyMinCheckoutPriceCondition($cartRef=null) {
		if(is_null($cartRef)) {
			global $woocommerce;
			$cartRef = $woocommerce->cart;
		}
	...
	public function ced_cwsm_min_checkout_price_module_add_setting( $settingReceived, $current_section ) {
		if($current_section == 'ced_cwsm_min_checkout_price_module') {
			$settings = apply_filters( 'ced_cwsm_min_checkout_price_module_setting', array(
