jQuery(document.body).on('click','button#ced_cwsm_send_mail',function(event){
	...
	if(suggestionTitle == "" || suggestionDetail == "")
	{
		...
		jQuery("div#ced_cwsm_mail_empty_msg").show().delay(2000).fadeOut(function(){
		});
    ...
    jQuery.ajax({
		...
		success : function( response ) 
		{
			...
			if(response == "success")
			{
				jQuery("div#ced_cwsm_mail_success_msg").show().delay(2000).fadeOut(function(){
			...
			{
				jQuery("div#ced_cwsm_mail_failure_msg").show().delay(2000).fadeOut(function(){
				});
...
jQuery(document.body).on('click','h3.ced_cwsm_suggestion_sticky_header',function(){
...
jQuery(document.body).on('click','.suggest_uus',function(){
