...
if( !class_exists( 'Wholesale_user_register_addon' ) )
{
	...
	class Wholesale_user_register_addon
	{
		...
		* @link http://www.cedcommerce.com/
		*/
		function ced_cwsm_add_request_addon_basic_section( $sections )
		{
		...
		* @link http://www.cedcommerce.com/
		*/
		function ced_cwsm_add_request_addon_basic_settings( $settingReceived, $current_section )
		{
			if($current_section == 'ced_wmra_role_request_section')
			{
				$settings = array(
					'request_addon_section_title' => array (
						'name' => __ ( 'Role Management Settings', 'wholesale-market' ),
						'type' => 'title',
						'desc' => __ ( '', 'wholesale-market' ),
		...
		function ced_wholesale_request_bulkedit_save($product)
		{
			if(isset($_REQUEST['ced_cwsm_wholesale_price']))
			{
				...
				$price = $_REQUEST['ced_cwsm_wholesale_price'];
			...
			if(isset($_REQUEST['ced_cwsm_min_qty_to_buy']))
			{
				...
				$qty = $_REQUEST['ced_cwsm_min_qty_to_buy'];
				update_post_meta($id, 'ced_cwsm_min_qty_to_buy', $qty);
			}
		}
		
		function ced_wholesale_request_bulkedit()
		{
			?>
			<label class="alignleft stock_qty_field">
				<span class="title"><?php _e('Wholesale Price', 'wholesale-market');?></span>
			...
			?>
			<label class="alignleft stock_qty_field">
				<span class="title"><?php _e('Wholesale Quantity', 'wholesale-market');?></span>
				<span class="input-text-wrap">
		...
		* @link http://www.cedcommerce.com/
		*/
		function ced_wholesale_request_send_callback()
		{
			if(isset($_POST['wholesale_request']) && !empty($_POST['wholesale_request']))
			{
				...
				{
					$select_data = $_POST['subscription_data'];
					...
					if ( get_option('ced_cwsm_request_role_addon_directly') == 'yes' ) {
						...
						$user_data->add_role( $_POST[ 'role_required' ] );
					}
					else{
						update_user_meta($current_user_id, 'ced_wholesale_request', $_POST['role_required']);
					}
		...
		* @link http://www.cedcommerce.com/
		*/
		function ced_wholesale_dashboard_request()
		{
			$current_user_id = get_current_user_id();
				...
				if( !$globalCWSM->isCurrentUserIsWholesaleUser() )
				{
					if ( !get_user_meta( $current_user_id, 'ced_wholesale_request', TRUE ) || get_user_meta( $current_user_id, 'ced_wholesale_request', TRUE ) == 'cancel' ) {
						...
						<div id="ced_dashboard_success_msg" style="display:none;">
							<span style="color:red !important;"><?php _e('Role Successfully Requested' , 'wholesale-market'); ?></span>
						</div>
						<?php $payment_addon_enable=get_option('ced_cwsm_basic_settings_payment_enable');
							if($payment_addon_enable=='on'){
							 ?>
						...
						<div class="ced_cwsm_request_role">							
							<p class="woocommerce-FormRow woocommerce-FormRow--wide form-row form-row-wide" style="padding-bottom:10px;">
								<select name="ced_cwsm_user_role" id="reg_role1">
									<option value=""><?php _e( 'Select Role' , 'wholesale-market' ) ?></option>
									<option value="ced_cwsm_wholesale_user"><?php _e( 'Wholesale Customer' , 'wholesale-market' ) ?></option>
							...
							<?php }else{ ?>
							<a class="ced_wm_wholesale_request1 button" style="padding:10px;" id="ced_wm_wholesale_request1" data-id="<?php echo $current_user_id;?>" href="javascript:void(0);"><?php _e('Click Here','wholesale-market'); ?></a>
							<?php } ?>
							<img src="<?php echo CED_CWSM_PLUGIN_DIR_URL.'assets/images/ajax-loader.gif'; ?>" style="display:none;" width="30px" height="30px" id="ced_cwsm_ajax_loader1" >
						</div>
					<?php
					}	
				}
		...
		*/
		...
		function ced_wura_checkout_register_script()
		{
			// Register the script
			wp_register_script('ced_wura_wholesale_user_addon_script', CED_WURA_DIR_URL.'/assets/js/wholesale-user-register.js', array('jquery'));
			...
			wp_enqueue_style( 'ced_wura_wholesale_user_addon_style', CED_WURA_DIR_URL.'/assets/css/wholesale-user-register.css' );
		}
		function ced_wholesale_show_msg_to_paynow(){
			$current_user_id = get_current_user_id();
			$current_user_info = get_userdata($current_user_id);
			$current_user_role = $current_user_info->roles;
				...
				/*if(get_user_meta( $current_user_id , 'ced_cwsm_request_for_role', TRUE)!='') {echo "hi";*/
					if(get_user_meta( $current_user_id , 'ced_cwsm_payement_status', TRUE)=='no') { ?>
						<div id="display_subs_msg">
							<p style="color:red !important;"><?php _e('Your Package will not be applied until you dont pay for your requested package. Please pay to enjoy your package' , 'wholesale-market'); ?></p>
							<p>Your selected package :<?php echo $package; ?></p>
							<p>Duration :<?php echo $months; ?>months and <?php echo $days; ?>days</p>
							<p>Price :<?php echo get_woocommerce_currency_symbol().$price; ?></p>
							<span><input type="submit" class="Button button" name="paynow" value="<?php esc_attr_e( 'Pay Now', 'wholesale-market' ); ?>" /></span>
						</div>
					<?php }elseif (get_user_meta( $current_user_id , 'ced_cwsm_payement_status', TRUE)=='yes') { ?>
							<p style="color:red !important;"><?php _e('Thanks for choosing this package. Now you are our wholesaleuser' , 'wholesale-market'); ?></p>
							<p>Your selected package :<?php echo $package; ?></p>
							<p>Package Duration :<?php echo $months; ?>months and <?php echo $days; ?>days</p>
							<p>Price :<?php echo get_woocommerce_currency_symbol().$price; ?></p>
					<?php }
				/*}*/
			}
