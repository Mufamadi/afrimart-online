...
class Wholesale_Request_Widget extends WP_Widget {
					...
					if ( !get_user_meta( $current_user_id, 'ced_wholesale_request', TRUE ) || get_user_meta( $current_user_id, 'ced_wholesale_request', TRUE ) == 'cancel' ) {
						...
						if ($check) 
						{
							...
							<div class="ced_cwsm_request_role">								
								<?php echo __( 'Want to Modify Role ', 'wholesale-market' ); ?>
								...
								<?php }else{ ?>
								<a class="ced_wm_wholesale_request button" style="padding:10px;" id="ced_wm_wholesale_request" data-id="'.$current_user_id.'" href="javascript:void(0);"><?php _e('Click Here','wholesale-market'); ?></a>
								<?php } ?>
								<img src="<?php echo CED_CWSM_PLUGIN_DIR_URL.'assets/images/ajax-loader.gif'; ?>" style="display:none;" width="30px" height="30px" id="ced_cwsm_ajax_loader">
							</div>
							<?php
	...
	public function form( $instance ) {
		...
		?>
		<p>
		<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( esc_attr( 'Title:', 'wholesale-market' ) ); ?></label> 
