...
function sw_ajax_search_admin_notice(){
	?>
	<div class="error">
		<p><?php esc_html_( 'SW Ajax WooCommerce Search is enabled but not effective. It requires WooCommerce in order to work.', 'sw_ajax_woocommerce_search' ); ?></p>
	</div>
<?php
}
...
class sw_woo_search_widget extends WP_Widget {
	...
	function __construct(){
		...
		if( version_compare( WC()->version, '2.4', '>=' ) ){
				add_action( 'wc_ajax_sw_search_products_callback', array( $this, 'sw_search_products_callback' ) );
		}else{
		...
		vc_map( array(
		  ...
		  "params" => array(
			...
			array(
				...
				"holder" => "div",
				"class" => "",
				"heading" => __( "Ajax Search", 'sw_ajax_woocommerce_search' ),
				"param_name" => "ajax_search",
				"value" => array('Yes' => 1, 'No' => 0),
				"description" => __( "Enable Ajax Search", 'sw_ajax_woocommerce_search' )
			 ),				 
			 array(
				"type" => "textfield",
				"holder" => "div",
	...
	**/
	...
	function sw_search_script(){
		...
		wp_register_script( 'sw_woocommerce_search_products', plugins_url('js/sw_woocommerce_search_products.min.js', __FILE__) );
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['ajax_search'] 		 = $new_instance['ajax_search'];
	...
	public function form( $instance ) {
		...
		$ajax_search 			= isset( $instance['ajax_search'] )    	? 	$instance['ajax_search'] : 1 ;		
		...
		<p>
			<label for="<?php echo $this->get_field_id('ajax_search'); ?>"><?php _e("Ajax Search", 'sw_ajax_woocommerce_search')?></label>
			<br/>			
			<select class="widefat"
				id="<?php echo $this->get_field_id('ajax_search'); ?>"	name="<?php echo $this->get_field_name('ajax_search'); ?>">
				<option value="1" <?php if ($ajax_search==1){?> selected="selected"
				<?php } ?>>
					<?php _e('Yes', 'sw_ajax_woocommerce_search')?>
				</option>	
				<option value="0" <?php if ($ajax_search==0){?> selected="selected"
				<?php } ?>>
					<?php _e('No', 'sw_ajax_woocommerce_search')?>
				</option>					
			</select>
