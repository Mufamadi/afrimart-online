...
*/
class Revo_Menu_Checkbox extends Walker_Nav_Menu{
...
function revo_check_wdisplay ($widget_display){
	...
	$Revo_Menu_Checkbox = new Revo_Menu_Checkbox;
					...
					if ( ($key == 'login' && $user_ID) || ($key == 'logout' && !$user_ID) ){
						...
						if (isset($widget_display['checkbox']['menus'])) {
							...
							foreach ($widget_display['checkbox']['menus'] as $menu_id => $item_ids) {
								...
								if ( $Revo_Menu_Checkbox->is_menu_item_active($menu_id, $item_ids) ) return true;
							}
						}
					}
				}
		...
		if (isset($widget_display['checkbox'])) {
			...
			if (isset($widget_display['checkbox']['menus'])) {
				...
				foreach ($widget_display['checkbox']['menus'] as $menu_id => $item_ids) {
					...
					if ( $Revo_Menu_Checkbox->is_menu_item_active($menu_id, $item_ids) ) return false;
				}
			}			
		} else return false ;
	}
...
**  Is active sidebar
*/
function revo_sidebar_check($index) {
		...
		foreach ($sidebars_widgets[$index] as $i => $id) {
			...
			if ( isset($wp_registered_widgets[$id]) ) {
				...
				if ( isset($instances) && isset($number) ) {
					...
					if ( isset($instance['widget_display']) && revo_check_wdisplay($instance['widget_display']) == false ) {
...
function revo_get_social() {
	...
	$social = revo_options()->getCpanelValue('social_share');	
...
function revo_content_product(){ 
	$left_span_class 			= revo_options()->getCpanelValue('sidebar_left_expand');
	$left_span_md_class 	= revo_options()->getCpanelValue('sidebar_left_expand_md');
	$left_span_sm_class 	= revo_options()->getCpanelValue('sidebar_left_expand_sm');
	$right_span_class 		= revo_options()->getCpanelValue('sidebar_right_expand');
	$right_span_md_class 	= revo_options()->getCpanelValue('sidebar_right_expand_md');
	$right_span_sm_class 	= revo_options()->getCpanelValue('sidebar_right_expand_sm');
	$sidebar 							= revo_options()->getCpanelValue('sidebar_product');
	if( !is_post_type_archive( 'product' ) ){
		$term_id = get_queried_object()->term_id;
		$sidebar = ( get_term_meta( $term_id, 'term_sidebar', true ) != '' ) ? get_term_meta( $term_id, 'term_sidebar', true ) : revo_options()->getCpanelValue('sidebar_product');
	}
...
function revo_content_product_detail(){
	...
	$right_span_md_class 	= revo_options()->getCpanelValue('sidebar_right_expand_md');
	$right_span_sm_class 	= revo_options()->getCpanelValue('sidebar_right_expand_sm');
	$sidebar_template 		= revo_options()->getCpanelValue('sidebar_product');
	
	if( is_singular( 'product' ) ) :
		$sidebar_template = ( get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) != '' ) ? get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) : revo_options()->getCpanelValue('sidebar_product');
...
function revo_content_blog(){
	...
	$right_span_md_class 	= revo_options()->getCpanelValue('sidebar_right_expand_md');
	$right_span_sm_class 	= revo_options()->getCpanelValue('sidebar_right_expand_sm');
	$sidebar_template 		= revo_options() -> getCpanelValue('sidebar_blog');
	$sidebar  						= 'left-blog';
	if( is_single() ) :
		$sidebar_template = ( strlen( get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) ) > 0 ) ? get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) : revo_options()->getCpanelValue('sidebar_blog');
...
function revo_sidebar_template(){
	$revo_sidebar_teplate = revo_options() -> getCpanelValue('sidebar_blog');
	if( !is_archive() ){
		$revo_sidebar_teplate = ( get_term_meta( get_queried_object()->term_id, 'term_sidebar', true ) != '' ) ? get_term_meta( get_queried_object()->term_id, 'term_sidebar', true ) : revo_options()->getCpanelValue('sidebar_blog');
	}	
	if( is_single() ) {
		$revo_sidebar_teplate = ( get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) != '' ) ? get_post_meta( get_the_ID(), 'page_sidebar_layout', true ) : revo_options()->getCpanelValue('sidebar_blog');
	}
...
function revo_typography_css(){
	...
	$webfont 				= ( $page_webfonts != '' ) ? $page_webfonts : revo_options()->getCpanelValue( 'google_webfonts' );
	$header_webfont = revo_options()->getCpanelValue( 'header_tag_font' );
	$menu_webfont 	= revo_options()->getCpanelValue( 'menu_font' );
	$styles = '<style>';
	if ( $webfont ):	
...
function revo_typography_css_cache(){ 
	...
	/* Custom Css */
	if ( revo_options()->getCpanelValue('advanced_css') != '' ){
		echo'<style>'. revo_options()->getCpanelValue( 'advanced_css' ) .'</style>';
	}
...
function revo_typography_webfonts(){
	...
	$webfont 				= ( $page_google_webfonts != '' ) ? $page_google_webfonts : revo_options()->getCpanelValue('google_webfonts');
	$header_webfont = revo_options()->getCpanelValue( 'header_tag_font' );
	$menu_webfont 	= revo_options()->getCpanelValue( 'menu_font' );
	...
	if ( $webfont || $header_webfont || $menu_webfont ):
		...
		$webfont_weight	= ( get_post_meta( get_the_ID(), 'webfonts_weight', true ) ) ? get_post_meta( get_the_ID(), 'webfonts_weight', true ) : revo_options()->getCpanelValue('webfonts_weight');
		$font_weight = '';
...
if( !is_admin() ){
	...
	function revo_direction( $doctype = 'html' ){
		$revo_direction = revo_options()->getCpanelValue( 'direction' );
...
if ( !function_exists( 'revo_breadcrumb' ) ) {
	...
	function revo_breadcrumb( $before = '', $after = '', $display = true ) {
		...
		/* Turn off Breadcrumb */
		if( revo_options()->getCpanelValue( 'breadcrumb_active' ) ) :
			$display = false;
		endif;
...
function revo_footer_advanced(){
	...
	** Back To Top 
	*/
	if( revo_options()->getCpanelValue( 'back_active' ) ) :
	...
	** Popup 
	*/
	if( revo_options()->getCpanelValue( 'popup_active' ) ) :
		$revo_content = revo_options()->getCpanelValue( 'popup_content' );
		$revo_shortcode = revo_options()->getCpanelValue( 'popup_form' );
		$popup_attr = ( revo_options()->getCpanelValue( 'popup_background' ) != '' ) ? 'style="background: url( '. esc_url( revo_options()->getCpanelValue( 'popup_background' ) ) .' )"' : '';
?>
...
function revo_advanced(){	
	$revo_popup	 		= revo_options()->getCpanelValue( 'popup_active' );
	$sticky_mobile	 		= revo_options()->getCpanelValue( 'sticky_mobile' );
	$output  = '';
	$output .= '(function($) {';
	if( !revo_mobile_check() ) : 
		$sticky_menu 		= revo_options()->getCpanelValue( 'sticky_menu' );
		$revo_header_style 	= ( get_post_meta( get_the_ID(), 'page_header_style', true ) != '' ) ? get_post_meta( get_the_ID(), 'page_header_style', true ) : revo_options()->getCpanelValue('header_style');
		$output_css = '';
		$layout = revo_options()->getCpanelValue('layout');
		$bg_image = revo_options()->getCpanelValue('bg_box_img');
		$header_mid = revo_options()->getCpanelValue('header_mid');
		$bg_header_mid = revo_options()->getCpanelValue('bg_header_mid');			
		...
		*/
		if( $sticky_menu ) :		
			...
			** Adnvanced JS
			*/
			if( revo_options()->getCpanelValue( 'advanced_js' ) != '' ) :
				$output .= revo_options()->getCpanelValue( 'advanced_js' );
			endif;
...
function revo_logo(){
	...
	$scheme 		 = ( $scheme_meta != '' && $scheme_meta != 'none' ) ? $scheme_meta : revo_options()->getCpanelValue( 'scheme' );
	$meta_img_ID = get_post_meta( get_the_ID(), 'page_logo', true );
	$meta_img 	 = ( $meta_img_ID != '' ) ? wp_get_attachment_image_url( $meta_img_ID, 'full' ) : '';
	$mobile_logo = revo_options()->getCpanelValue( 'mobile_logo' );
	$logo_select = ( revo_mobile_check() && $mobile_logo != ''  ) ? $mobile_logo : revo_options()->getCpanelValue( 'sitelogo' );
...
function post_init(){
	add_meta_box("revo_product_meta", esc_html__( 'Recommend Product:', 'revo' ), "revo_product_meta", "product", "normal", "low");
...
function revo_product_save_meta(){
	global $post;
...
function revo_blogcol(){
	global $sw_blogcol;
	$blog_col = ( isset( $sw_blogcol ) && $sw_blogcol > 0 ) ? $sw_blogcol : revo_options()->getCpanelValue('blog_column');
...
function revo_trim_words( $title ){
	$title_length = intval( revo_options()->getCpanelValue( 'title_length' ) );
	$html = '';
	if( $title_length > 0 ){
...
function revo_site_favicon( $url ){
	if ( revo_options()->getCpanelValue('favicon') ){
		$url = esc_url( revo_options()->getCpanelValue('favicon') );
	}
	return $url;
}
...
** Social Link
*/
function revo_social_link(){
	$fb_link = revo_options()->getCpanelValue('social-share-fb');
	$tw_link = revo_options()->getCpanelValue('social-share-tw');
	$tb_link = revo_options()->getCpanelValue('social-share-tumblr');
	$li_link = revo_options()->getCpanelValue('social-share-in');
	$gg_link = revo_options()->getCpanelValue('social-share-go');
	$pt_link = revo_options()->getCpanelValue('social-share-pi');
	$it_link = revo_options()->getCpanelValue('social-share-instagram');
