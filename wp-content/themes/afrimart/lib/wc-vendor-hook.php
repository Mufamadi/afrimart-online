...
if ( WC_Vendors::$pv_options->get_option( 'sold_by' ) ) { 
	...
	add_action( 'woocommerce_after_shop_loop_item', 'revo_soldby_wrapper_start', 1 );
	add_action( 'woocommerce_after_shop_loop_item','revo_soldby_wrapper_end', 51 );
	function revo_soldby_wrapper_start(){
		echo '<div class="wc-soldby-start">';
	}
	
	function revo_soldby_wrapper_end(){
		echo '</div>';
	}
} 