...
function revo_widgets_init() {
	...
	register_widget('Revo_Posts_Widget');
 ...
 * @since 2.8.0
*/
class Revo_Posts_Widget extends Revo_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'sw_posts', 'description' => esc_html__('SW Posts', 'revo'));
		parent::__construct('sw_posts', esc_html__('SW Posts', 'revo'), $widget_ops);
...
class Revo_Widgets{
	...
	public function in_widget_form($widget, $r = null, $instance = array() ){
		...
		$widget_advanced = revo_options()->getCpanelValue('widget-advanced');
		...
		<div class="advanced-opt <?php echo $adoptions_on_class; ?>">
			<div class="advanced-opt-controls" >
				<label class="button toggle" for="<?php echo esc_attr( $widget->get_field_id('adoptions') ); ?>"><?php esc_html_e( 'Advanced Options', 'revo' ) ?>
			...
			<div class="advanced-opt-pane">
				<div class="advanced-opt-pane-inner">
					<div class="pane-content">
						<div class="pane-left">
							<p>
								<label for="<?php echo esc_attr( $widget->get_field_id('widget_style') ); ?>"><?php esc_html_e( 'Widget Style', 'revo' ) ?>
	...
	private function getDisplaySelectOptions(){
		if ( is_null($this->display_select_tpl) ){
			$this->display_select_options = $display_select_options = array(
				'all' => esc_html__('All', 'revo'),
				'if_selected' => esc_html__('Only on selected', 'revo'),
				'if_no_selected' => esc_html__('Except selected', 'revo')
			);
			$html = '';
			$html .= '<p>';
			$html .= '<label for="id-display-select">'. esc_html__( 'Widget Display', 'revo' ) .  '</label>';
	...
	private function getDisplayCheckboxUsers(){
		if ( is_null($this->display_checkbox_users_tpl) ){
			$this->display_checkbox_users = $display_checkbox_users = array(
				'login' => esc_html__('Show only for login users' , 'revo'),
				'logout' => esc_html__('Show only for logout users' , 'revo')
			);
	...
	private function getDisplayLanguage(){
		...
		$html = '';
		$html .= '<p>';
		$html .= '<label for="id-display-language">'. esc_html__( 'Language Display', 'revo' ) .  '</label>';
	...
	private function getDisplayCheckboxGeneral(){
		if ( is_null($this->display_checkbox_general_tpl) ){
			$this->display_checkbox_general = $display_checkbox_general = array(
					'home' => esc_html__('Blog posts', 'revo'),
					'archive' => esc_html__('Archive page', 'revo'),
					'date' => esc_html__('Archive Date', 'revo'),
					'category' => esc_html__('Category archive', 'revo'),
					'tag' => esc_html__('Tag archive', 'revo'),
					'author' => esc_html__('Author archive', 'revo'),
					'search' => esc_html__('Search page', 'revo'),
					'single' => esc_html__('Single post', 'revo'),
					'page' => esc_html__('Page template', 'revo'),
					'404' => esc_html__('Page 404', 'revo')
				);
			$html = '';
				...
				if(!isset($menu) || $menu != NULL || $menu != "")
					{
						...
						if (count($items) > 0) {
							$menu_checkbox = new Revo_Menu_Checkbox( $menu->slug);
	...
	protected function getWidgetStyles(){
		...
		if ( is_null($this->styles) ){
			$tmp = array();
			if ( $_core_styles = $this->scandir(REVO_DIR.'/widgets/_styles', 'php') )
	...
	protected function loadStyle( $style = '' ){
		...
		if ( !empty($style) ){
			...
			$_core_style = REVO_DIR.'/widgets/_styles/'.$style.'.php';
